...
async function loadData() {
  try {
    const [configRes, historyRes, appearanceRes] = await Promise.all([
      browser.runtime.sendMessage({ action: 'getConfig' }),
      browser.runtime.sendMessage({ action: 'getHistory' }),
      browser.runtime.sendMessage({ action: 'getAppearance' })
    ]);
    ...
    if (appearanceRes.success && appearanceRes.data) {
      state.appearance = appearanceRes.data;
    }
...
function applyAppearance() {
  ...
  document.documentElement.style.setProperty('--ai-secondary', themeColors.secondary);
  document.documentElement.style.setProperty('--ai-gradient', `linear-gradient(135deg, ${themeColors.primary} 0%, ${themeColors.secondary} 100%)`);
  document.documentElement.style.setProperty('--ai-shadow', `${themeColors.primary}66`);
  document.documentElement.style.setProperty('--ai-shadow-hover', `${themeColors.primary}80`);
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // 监听外观更新
  if (message.action === 'updateAppearance' && message.appearance) {
